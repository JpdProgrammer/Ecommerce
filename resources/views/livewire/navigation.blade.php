<header class="bg-trueGray-700 sticky top-0" style="z-index: 900" x-data="dropdown()">
    <div class="container flex items-center h-16 justify-between md:justify-start">
        <a dusk="categoryNavigationBtn" :class="{'bg-opacity-100 text-orange-500': open}" x-on:click="show()" class="flex flex-col items-center justify-center order-last md:order-first px-6 sm:px-4 bg-white bg-opacity-25 text-white cursor-pointer font-semibold h-full">
        ...
        <!-- Settings Dropdown -->
        <div dusk="menuUserIcon" class="mx-6 relative hidden md:block">
            @auth
            ...
            <x-jet-dropdown align="right" width="48">
                ...
                <x-slot name="content">
                    ...
                    <!-- Authentication -->
                    <form dusk="logoutLink" method="POST" action="{{ route('logout') }}">
                        @csrf
        ...
        <div dusk="shoppingCartIconMenu" class="hidden md:block">
    ...
    <nav id="navigation-menu" x-show="open"
        ...
        <div class="container h-full hidden sm:block">
            <div dusk="subcategoryListNavigation" x-on:click.away="close()" class="grid grid-cols-4 h-full">
                <ul class="bg-white">
                    @foreach($categories as $category)
                        <li class="navigation-link text-trueGray-500 hover:bg-orange-500 hover:text-white">
                            <a dusk="{{ 'category_' . $category->id }}" href="{{ route('categories.show', $category) }}" class="py-2 px-4 text-sm flex items-center">
